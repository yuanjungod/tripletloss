...
class TripletSelectLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.triplet = int(config.BATCH_SIZE / 3)
        print(type(self.triplet))
        top[0].reshape(self.triplet, int(shape(bottom[0].data)[1]))
        top[1].reshape(self.triplet, int(shape(bottom[0].data)[1]))
        top[2].reshape(self.triplet, int(shape(bottom[0].data)[1]))
