...
from numpy import *
import yaml
from tripletloss.sampledata import SampleData
import random
import cv2
from tripletloss.blob import prep_im_for_blob, im_list_to_blob
...
class DataLayer(caffe.Layer):
    ...
    def _get_next_minibatch(self):
        ...
        sample = []
        if self._index >= len(self.data_container.sample):
            self._index = 0
        archor = self.data_container.sample[self._index]
        archor_personname = archor.split('@')[0]
        self._index = self._index + 1
        while len(sample) < self.triplet:
            # print("fuck1")
            sample.append(archor)
        # Sample positive samples
        while len(sample) < self.triplet * 2:
            picindex = random.randint(0, len(self.data_container.sample_person[archor_personname]) - 1)
            if self.data_container.sample_person[archor_personname][picindex] not in sample:
                sample.append(self.data_container.sample_person[archor_personname][picindex])
        # Sample negative samples
        while len(sample) < self.triplet * 3:
            # print("fuck3")
            rand = random.randint(0, len(self.data_container.sample_person) - 1)
            # print(type(self.data_container._sample_person.keys()))
            personname = list(self.data_container.sample_person.keys())[rand]
            if archor_personname == personname:
                ...
                if index == 0:
                    ...
                    index = rand - 1
                # print(len(list(self.data_container._sample_person.keys())))
                personname = list(self.data_container.sample_person.keys())[index]
            picindex = random.randint(0, len(self.data_container.sample_person[personname]) - 1)
            if self.data_container.sample_person[personname][picindex] not in sample:
                sample.append(self.data_container.sample_person[personname][picindex])
    ...
    def setup(self, bottom, top):
        ...
        self._name_to_top_map = {'data': 0, 'labels': 1}
...
class TestBlobFetcher():
    ...
    def _get_next_minibatch(self):
        ...
        while len(sample) < 3 * self._triplet:
            rand = random.randint(0, len(self.data_container.sample_person) - 1)
            personname = self.data_container.sample_person.keys()[rand]
            if archor_personname == personname:
                index = max(0, rand - 1)
                if index == 0:
                    index = rand + 1
                    ...
                    index = rand - 1
                personname = self.data_container.sample_person.keys()[index]
            picindex = random.randint(0, len(self.data_container.sample_person[personname]) - 1)
            if (self.data_container.sample_person[personname][picindex]) not in sample:
                sample.append(self.data_container.sample_person[personname][picindex])
